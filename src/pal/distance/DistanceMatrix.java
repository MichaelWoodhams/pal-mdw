...
public class DistanceMatrix implements Serializable, IdGroup
{
	...
	/**
	 * Prints in an attractive format, but will be too wide for large matrices.
	 * TODO: take column width as an argument.
	 * @param out
	 */
	public void printPretty(PrintWriter out) {
		int n = distance.length;
		String[] idStrings = new String[n];
		
		int maxLen = 0;
		for (int i=0; i<n; i++) {
			idStrings[i] = idGroup.getIdentifier(i).getName();
			maxLen = pal.math.MathUtils.max(maxLen,idStrings[i].length());
		}
		String labelFormat = "%" + Integer.toString(maxLen) + "s ";
		out.printf(labelFormat, "");
		for (int col=0; col<n; col++) {
			out.printf("%7s ", idStrings[col]);
		}
		out.println();
		for (int row=0; row<n; row++) {
			out.printf(labelFormat, idStrings[row]);
			for (int col=0; col<n; col++) {
				out.printf("%7.4f ", distance[row][col]);
			}
			out.println();
		}
	}
	...
	/**
	 * Returns the sum of distances in a distance matrix
	 * (each distance counted only once - i.e. sum of above diagonal elements)
	 * @return sum
	 */
	...
	/**
	 * Multiplies all distances by scale.
	 * @param scale
	 */
