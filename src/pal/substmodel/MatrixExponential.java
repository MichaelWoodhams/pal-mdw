 ...
 * 
 * Comments added by Michael Woodhams (comments labeled 'MDW'). As I'm not the author, these are not authoritative.
 * 
 * @deprecated by MDW - this class produces erroneous results when given a matrix with complex
 * eigenvalues. Use EigenSystem instead.
 */
@Deprecated 
...
//
	...
	private final double[] Eval;   // MDW: Eigenvalues of input matrix
	private final double[][] Evec; // MDW: Columns are Eigenvectors of input matrix
	private final double[][] Ievc; // MDW: inverse of Evec
	private final double[][] iexp; // MDW: work space - contains nothing of value between function calls
	private final int[] ordr;      // MDW: pivoting?? Written by elmhes, read only by elttran, so it is also working space only?
	private final double[] evali;  // MDW: ? work space passed to hqr2 routine?
	private final double amat[][]; // MDW: initially the input matrix - used as working space only.
	...
	/**
	 * MDW: It looks like this routine effectively takes a matrix logarithm of its input
	 * and stores that as the new rate matrix. 
	 * 
	 * Given matrix A (amat), find (and remember) E (Evec) matrix which has A's e-vectors
	 * as its columns, and d (Eval) the vector of eigenvalues, and E^-1 (Ievc), the inverse of E.
	 * Then if D is diagonal matrix of d, we have A = E D E^-1 and A^n = E D^n E^-1.
	 * Hence we can raise A to an arbitrary power easily, and also for constant c
	 * exp(c A) = E exp(c D) E^-1, and exp(c D) is very easy to calculate as D is diagonal.
	 * 
	 * I think ordr is for pivoting.
	 * 
	 */
	...
	/**
	 * MDW: Apparently from 'Numerical Recipes', section 11.5. 
	 * Convert real non-symmetric matrix a to one which has the same eigenvalues but is in Hessenberg form
	 * (lower left triangle is zeroed except for the first subdiagonal.) 
	 * (Or possibly from eispack: http://www.netlib.org/eispack/elmhes.f)
	 */
	...
	// MDW: global variables shared between mcdiv and hqr2 - presumably because Java doesn't easily allow mcdiv to return
	// two values.
	...
	/**
	 * MDW:
	 * Possibly from http://www.netlib.org/eispack/hqr2.f
	...
	/**
	 * MDW:
	 * From the eispack routine elmhes.f?
	 * (Not in Numerical Recipes.) 
	 ...
	 * (This implementation seems to have omitted the 'low' and '(h)igh' arguments and so transforms
	 * the entire matrix.)
	 */
	...
	/**
	 * MDW: Not in Numerical Recipes or in eispack. It looks to calculate the inverse of
	 * inmat and put the result in imtrx, using LU decomposition for the calculation.
	 * See for example http://oregonstate.edu/~hohenlop/MatrixFunx.h
	 * Possibly from LAPACK or a C translation of LAPACK?
	 * http://osirus.usask.ca/osiris/software/documentation/library/nxbase/html/classLapackMatrix.html
	 */
