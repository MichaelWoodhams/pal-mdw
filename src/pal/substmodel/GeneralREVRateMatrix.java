...
public class GeneralREVRateMatrix implements NeoRateMatrix {
	...
	// MDW: Added ability to set upper bounds on parameters.
	private final static int DEFAULT_UPPER_BOUND = 1000;
	private final double[] upperBounds_;
	
	// MDW: Print the object in a user-readable manner (e.g. for debugging)
	// 'ps' (prefix string) allows formatting of nested objects.
	public void printObject(PrintStream out) {this.printObject(out,"");}
	public void printObject(PrintStream out, String ps) {this.printObject(out,ps,defaultParameters_);}
	public void printObject(PrintStream out, String ps, double[] parameters) {
		out.printf(ps+"Object: GeneralREVRateMatrix, dimension = %d, number effective parameters = %d\n",dimension_,numberOfEffectiveParameters_);
		out.print(ps+"parameterChecks_:\n"+ps);
		// TODO: Pretty this up. Print '.' on diagonal, letters ("a" instead of "0" etc) on upper right, either "." or reflection of letters on lower left.
		for (int from = 0; from < dimension_; from++) {
			for (int to = 0; to < dimension_; to++) {
				out.printf("%3d%s", parameterChecks_[from][to], (to+1 == dimension_) ? "\n"+ps : ", ");
			}
		}
		double[][] rateStore = new double[dimension_][dimension_];
		createRelativeRates(rateStore, parameters, 0);
		out.print("Rate matrix:\n"+ps);
		for (int from = 0; from < dimension_; from++) {
			for (int to = 0; to < dimension_; to++) {
				System.out.printf("% 8f%s", rateStore[from][to], (to+1 == dimension_) ? "\n"+ps : ", ");
			}
		}
	...
	*
	 ...
	 */
	 public GeneralREVRateMatrix(int dimension, int[] constraints, double[] specifiedDefaultParameters, int fixedConstraintValue) {
		...
		this.upperBounds_ = new double[numberOfEffectiveParameters_];
		for (int i=0; i<numberOfEffectiveParameters_; i++) this.upperBounds_[i] = DEFAULT_UPPER_BOUND;
		//System.out.println("Number of effective parameters:"+numberOfEffectiveParameters_);
	}
	 ...
	 * @return true
	 */
	public boolean isIndependentOfEqbmFreq() { return true; }
	...
	public void setUpperBounds(double[] bounds) {
		System.arraycopy(bounds,0,upperBounds_,0,numberOfEffectiveParameters_);
	};
	public void setUpperBounds(double bound) {
		for (int i=0; i<numberOfEffectiveParameters_; i++) upperBounds_[i] = bound;
	}
// ============================================================================================
// === Utility Methods
	/**
	...
	/**
	 * Create a rate matrix equivalent to the Jukes Cantor model. Has no parameters.
	 * @return
	 */
	public static final GeneralREVRateMatrix createJC() {
		return new GeneralREVRateMatrix(4,new int[] {0, 0, 0, 0, 0, 0},new double[] {},0);
	}
}
