 ...
 * 
 * MDW: Modified Boltzman to take stabilization temp as a parameter (and corrected spelling)
*/
...
public interface ProbabilityIterator {
	...
	public static class Utils {
		...
		public static final Factory getBoltzman(double initialTemperature, double temperatureDecay, int chainLength) {
			return getBoltzman(initialTemperature,temperatureDecay,0.005,chainLength);
		}
		public static final Factory getBoltzman(double initialTemperature, double temperatureDecay, double stabilizationTemp, int chainLength) {
			return new Boltzman.PIFactory(initialTemperature,temperatureDecay,stabilizationTemp,chainLength);
		}
		//=============================================================
		...
		private static class Boltzman implements ProbabilityIterator{
			...
			int chainPosition_;
			double stabilizationTemp_; // Defaults to 0.005
			public Boltzman(double initialTemperature, double temperatureDecay, double stabilizationTemp, int chainLength) {
				this.initialTemperature_ = initialTemperature;
				this.temperatureDecay_ = temperatureDecay;
				this.stabilizationTemp_ = stabilizationTemp;
				this.chainLength_ = chainLength;
				this.currentTemperature_ = initialTemperature_;
				this.chainPosition_ = 0;
			}
			public boolean isStablised() { return currentTemperature_<stabilizationTemp_; }
			...
			static class PIFactory implements Factory {
				...
				int chainLength_;
				double stabilizationTemp_; // Defaults to 0.005
				public PIFactory(double initialTemperature, double temperatureDecay, double stabilizationTemp, int chainLength) {
					this.initialTemperature_ = initialTemperature;
					this.temperatureDecay_ = temperatureDecay;
					this.stabilizationTemp_ = stabilizationTemp;
					this.chainLength_ = chainLength;
				...
				public ProbabilityIterator newInstance() {
					return new Boltzman(initialTemperature_,temperatureDecay_,stabilizationTemp_,chainLength_);
				}
			}
		}//End of boltzman
	}
