...
public class AlignmentUtils {
	...
	/** Print alignment (in FASTA format) (unchunked, 50 per line)*/
	public static void printFASTA(Alignment a, PrintWriter out) {
		printFASTA(a, out, 50, false);
	}
	
	/** Print alignment (in FASTA format) */
	public static void printFASTA(Alignment a, PrintWriter out, int lineLength, boolean chunked) {
		
		int seqLen = a.getSiteCount();
		for (int s = 0; s < a.getSequenceCount(); s++)
		{
			out.println(">"+a.getIdentifier(s).getName());
			int n = 0;
			while (n<seqLen) {
				printNextSites(a, out, chunked, s, n, lineLength);
				out.println();
				n+=lineLength;
			}
		}
	...
	/** Print alignment (in FASTA format) for a subset of the sequences  (unchunked, 50 per line)*/
	public static void printFASTA(Alignment a, int[] subset, PrintWriter out) {
		printFASTA(a, subset, out, 50, false);
	}

	
	/** Print alignment (in FASTA format) for a subset of the sequences */
	public static void printFASTA(Alignment a, int[] subset, PrintWriter out, int lineLength, boolean chunked) {
		
		int seqLen = a.getSiteCount();
		for (int i = 0; i < subset.length; i++) {
			int seq = subset[i];
			out.println(">"+a.getIdentifier(seq).getName());
			int n = 0;
			while (n<seqLen) {
				printNextSites(a, out, chunked, seq, n, lineLength);
				out.println();
				n+=lineLength;
			}
		}
	 ...
	 *  set the frequencies internally for the alignment
	 *  
	 *  Adapted from estimateFrequencies(Alignment a) by MDW.
	 *  TODO: change estimateFrequencies(Alignment a, int sequence) to call
	 *  this method, to avoid duplication of code.
	 */
	public static double[] estimateFrequencies(Alignment a, int sequence)
	{
		...
		int[] stateCount = countStates(a,sequence);
	 ...
	 *  
	 */
	public static int[] countStates(Alignment a, int sequence)
	{
		DataType dt = a.getDataType();
		if (dt == null)		{
			dt = getSuitableInstance(a);
		...
		int[] stateCount = new int[numStates+1];
		...
		for (int j = 0; j < a.getSiteCount(); j++)
		{
			int state = dt.getState(a.getData(sequence,j));
			if(dt.isUnknownState(state)) {
				state = stateCount.length -1;
			}
			stateCount[state] += 1;
	...
	*/
	...
	@teturn the number of invalid characters
	*/
	public static final int countUnknowns(Alignment a, DataType dt, int sequence) {
		int count = 0;
		for(int i = 0 ; i < a.getSiteCount() ; i++) {
			if(dt.isUnknownState(dt.getState(a.getData(sequence,i)))) {
				count++;
			}
		}
		return count;
