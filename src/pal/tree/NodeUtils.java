...
import java.util.*;
import pal.io.*;
import pal.substmodel.ModelEdgeParameters;
import pal.util.*;
...
public class NodeUtils {
	...
	/**
	 * prints node in New Hampshire format.
	 * 
	 * This is the simplest version, omitting almost all the boolean arguments.
	 */
	public static void printNH(PrintWriter out, Node node,
		boolean printLengths, boolean printInternalLabels) {

		printNH(out, node, printLengths, printInternalLabels, false, false, false, 0, true);
	}


	/**
	 * Simpler version, omitting rarer boolean arguments
	 * @param out
	 * @param node
	 * @param printLengths
	...
	public static int printNH(PrintWriter out, Node node,
		...
		return printNH(out, node, printLengths, printInternalLabels, false, false, false, column, breakLines);
	}
	
	// MDW: Modified to have the option to print ModelEdgeParameters and bootstrap support on each edge.
	// Bootstrap support can be printed in two ways (controlled by printBootstrapsAsInternalLabels)
	// so tree can be printed as:
	...
	public static int printNH(
		...
		if (printInternalLabels && printBootstrapSupport && printBootstrapsAsInternalLabels) {
			throw new IllegalArgumentException("Can't print internal labels and also bootstraps as internal labels");
		}
		...
		if (!node.isLeaf())
		{
			...
			for (int i = 0; i < node.getChildCount(); i++)
			{
				...
				column = printNH(out, node.getChild(i), printLengths, printInternalLabels, printEdgeParameters, printBootstrapSupport, printBootstrapsAsInternalLabels, column, breakLines);
			}
		...
		if (!node.isRoot())
		{
			...
			if (printBootstrapSupport && printBootstrapsAsInternalLabels && !node.isLeaf()) {
				
				Double bootstrap = (Double)((AttributeNode)node).getAttribute(BootstrapAnalysis.BOOTSTRAP_ATTRIBUTE);
				// Java floating to fixed casts always round towards zero, so +0.5 to round it
				String support = Integer.toString((int)(bootstrap.doubleValue()+0.5)); 
				out.print(support);
			...
			// MDW: The modification for printing ModelEdgeParameters
			if (printEdgeParameters && node instanceof AttributeNode) {
				AttributeNode aNode = (AttributeNode) node;
				Object obj = aNode.getAttribute(ModelEdgeParameters.ATTRIBUTE_LABEL);
				if (obj instanceof ModelEdgeParameters) {
					ModelEdgeParameters mep = (ModelEdgeParameters) obj;
					String mepString = mep.toString();
					if (breakLines) column = breakLine(out, column);
					out.print(mepString);
					column += mepString.length();
				}
			}
		}
