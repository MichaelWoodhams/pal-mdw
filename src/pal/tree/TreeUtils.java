...
public class TreeUtils
{
	 ...
	 * 
	 * There is a bug: see source code for details.
	 * Workaround: only give fully resolved (i.e. binary) trees to this routine.
	 */
	...
	*/
	...
	public static boolean sameUnrootedTopology(Tree t1, Tree t2) {
		return (getRobinsonFouldsDistance(t1,t2) == 0);
	}
	...
	// Never use a RNG where you don't have the option of setting the seed.
	public static void setGetRandomNodeSeed(long seed) {
		random.setSeed(seed);
	}
	...
	/**
	 * print this tree in New Hampshire format, possibly with bootstrap supports
	 * 
	 * @param tree
	 * @param out output stream
	 * @param printLengths boolean variable determining whether
	 ...
	 * @param printInternalLabels boolean variable determining whether
	 *		internal labels should be included in output
	 * @param printEdgeParameters boolean - should (non-length, non-bootstrap) edge parameters be printed?
	 * @param printBootstrapSupport boolean - should bootstrap be printed?
	 * @param printBootstrapsAsInternalLabels - bootstrap in "(A:1,B:2)50:3," 
	 *      format or "(A:1,B:2):3[50]" format?
	 * @param column How wide are columns (for linebreaks?)
	 * @param breakLines Have linebreaks?
	 */
	public static void printNH(
		    ...
		    boolean printEdgeParameters,
		    boolean printBootstrapSupport,
		    boolean printBootstrapsAsInternalLabels,
		    int column, 
		    boolean breakLines) {

		NodeUtils.printNH(out, tree.getRoot(),	printLengths, printInternalLabels, 
				printEdgeParameters, printBootstrapSupport,
				printBootstrapsAsInternalLabels, column, breakLines);
		out.println(";");
	}
...
//	}
	...
	/**
	 * Reroots the tree at the given
	 * internal node. The resulting root node will have at least 3 children.
	 * If the current root is binary, the total number of nodes in the tree will
	 * decrease by one.
	 * @deprecated use TreeManipulator.getRootedAbove or TreeManipulator.getRootedBy
	 */
	public static void reroot(Tree tree, Node node) {
		reroot(node, null);
		// now 'node' either is the root, or (if it is a leaf) is child to the root
		Node root;
		if (node.isRoot()) { root = node; } else { root = node.getParent(); }
		tree.setRoot(root);
	}
	/**
	 * Reroots the tree to a node inserted into the edge above the given node.
	 * The resulting root node will have exactly two children. This may increase the total 
	 * number of nodes in the tree by one.
	 * 	@deprecated use TreeManipulator.getRootedAbove or TreeManipulator.getRootedBy
	 */
	public static void rerootBinary(Tree tree, Node node) {
		// creates a new root node as parent to the argument node:
		rerootBinary(node);
		tree.setRoot(node.getParent());
	}

	/**
	 * Added by MDW.
	 * Modify tree to be binary by (if necessary) adding new nodes with length = 0.
	 * Nodes with one child are not affected, so if the input tree has such nodes
	 * it won't strictly be binary.
	 * TODO: inverse operation: collapse nodes into their parent when their length = 0.
	 * 
	 * @param tree
	 */
	public static void forceBinary(Tree tree)  {
		forceBinary(tree.getRoot());
		tree.createNodeList();
	}
	public static void forceBinary(Node node) {
		int n = node.getChildCount();
		// The recursive step:
		for (int i=0; i<n; i++) {
			forceBinary(node.getChild(i));
		}
		if (n>2) {
			// Too many children at this node: reduce number by one
			Node newNode = node.getInstance();
			newNode.setBranchLength(0);
			// Transfer all but one of the current node's children to the new node
			for (int i=1; i<n; i++) {
				newNode.addChild(node.removeChild(0));
			}
			// and attach it
				node.addChild(newNode);
			// and if this node had four or more children, need to process newNode
			// as it has 3+ children
			if (n>3) {
				forceBinary(newNode);
			}
		}
	...
	/**
	 * 
	 * @param newRoot Reroot the tree to this internal node
	 * @param spareNode an optional 'spare node': this just gets passed through for possible use
	 * by rerootBinary
	 * @return possibly returns a now-unused node, for possible use by rerootBinary.
	 * @deprecated use TreeManipulator.getRootedAbove or TreeManipulator.getRootedBy
	 */
	private static Node reroot(Node newRoot, Node spareNode) {
		if (newRoot.isRoot()) {
			// no rerooting is necessary.
			return spareNode;
		}
		if (newRoot.getChildCount() == 1) {
			throw new RuntimeException("Found an illegal node - only one child");
		}
		if (newRoot.isLeaf()) {
			// if our 'root' is a leaf, we have no option but to insert a new root node into
			// its parent edge.
			rerootBinary(newRoot);
			return spareNode;
		}
		// Assertion: newRoot has at least two children
		Node oldRoot = newRoot.getParent(); // not the root yet...
		spareNode = reroot(oldRoot, spareNode); // but now it is.
		// Assertion: oldRoot has at least two children
		if (oldRoot.getChildCount() == 2) {
			// Special case: oldRoot is binary. In this case, it needs to be removed
			Node sister = oldRoot.getChild(0);
			if (sister == newRoot) {
				sister = oldRoot.getChild(1);
			}
			// Now I adopt my sister
			NodeUtils.removeChild(oldRoot, sister);
			newRoot.addChild(sister);
			// but any branch length I had to my old parent has to be transfered to my new daughter
			sister.setBranchLength(sister.getBranchLength()+newRoot.getBranchLength());
			// Assume standard errors are uncorrelated
			double newSE = Math.sqrt(MathUtils.square(sister.getBranchLengthSE())+MathUtils.square(newRoot.getBranchLengthSE()));
			sister.setBranchLengthSE(newSE);
			newRoot.setBranchLength(0);
			newRoot.setBranchLengthSE(0);
			oldRoot.removeChild(0); // remove old root's last child (newRoot)
			return oldRoot; // oldRoot is now a spare node.
		}
		// The 'normal' case: current parent oldRoot has 3 or more edges, so all we need do is reverse the direction
		// of the edge between newRoot and oldRoot.
		// Exchange branch label, length et cetera
		NodeUtils.exchangeInfo(oldRoot,newRoot);

		// Rearrange topology
		NodeUtils.removeChild(oldRoot, newRoot);
		newRoot.addChild(oldRoot);
		return spareNode;
	}
	
	/**
	 * Reroot the tree so that the root node is binary and has targetRoot as one of its direct children
	 * (I.e. insert a root node on the edge between targetRoot and its parent.)
	 * @param targetRoot
	 * @deprecated use TreeManipulator.getRootedAbove or TreeManipulator.getRootedBy
	 */
	private static void rerootBinary(Node targetRoot) {
		if (targetRoot.isRoot()) {
			throw new RuntimeException("Attempted rerootBinary on node which is already root");
		}
		Node parent = targetRoot.getParent();
		if (parent.getChildCount() == 1) {
			throw new RuntimeException("Found an illegal node - only one child");
		}
		if (parent.isRoot() && parent.getChildCount() == 2) {
			// nothing to do
			return;
		}
		Node newRoot = reroot(parent, null);
		if (newRoot == null) {
			newRoot = targetRoot.getInstance();
		}
		// up to this point, assignments to 'newRoot' are just about getting a Node object,
		// reusing a now unneeded one if necessary. It contains no useful information.
		NodeUtils.removeChild(parent, targetRoot);
		newRoot.addChild(parent);
		newRoot.addChild(targetRoot);
	}
	...
	// TODO (possibly): allow this to apply to just internal or just external branches
	public static void setMinBranchLength(Tree tree, double minBranch) {
		int n = tree.getExternalNodeCount();
		for (int i=0; i<n; i++) {
			Node node = tree.getExternalNode(i);
			if (node.getBranchLength()<minBranch) node.setBranchLength(minBranch);
		}
	...
	public static void setMaxBranchLength(Tree tree, double maxBranch) {
		int n = tree.getExternalNodeCount();
		for (int i=0; i<n; i++) {
			Node node = tree.getExternalNode(i);
			if (node.getBranchLength()>maxBranch) node.setBranchLength(maxBranch);
		}
		n = tree.getInternalNodeCount();
		for (int i=0; i<n; i++) {
			Node node = tree.getInternalNode(i);
	...
	/**
	 * Check tree to see if all the links are consistent.
	 * TODO: (perhaps) Check labels are unique, edge lengths non-negative.
	 * If there are any loops, they will show up as a violation of parent/child links pointing to each other.
	 * @param tree
	 * @return
	 */
	public static boolean isTreeValid(Tree tree) {
		return isSubtreeValid(tree.getRoot(), null);
	}
	private static boolean isSubtreeValid(Node treeRoot, Node parent) {
		if (treeRoot.getParent() != parent) { return false; }
		boolean valid = true;
		int n = treeRoot.getChildCount();
		for (int i=0; valid && i<n; i++) {
			valid = isSubtreeValid(treeRoot.getChild(i),treeRoot); // loop terminates if false.
		}
		return valid;
	}

	public static final void test() {
		// Run some tests. Currently only tests the 'reroot' operation, and presents results to stdout 
		// rather than allowing for automated testing. Also does not test error conditions (non-valid tree).
		// To automate tests, consider using distance matricies which should be unaffected by all of this.
		// 8 tests: start tree has ternary or binary root, rerooting to internal or leaf node, using 'reroot' or 'rerootBinary' to do it.
			
		PrintWriter stdout = new PrintWriter(System.out, true);
		Tree ternaryRootTree = stringToTree("(((A:1,B:2):3,C:4):5,D:6,E:7);");
		Tree binaryRootTree = stringToTree("(((A:1,B:2):3,C:4):5,D:6);");
		Tree rerootTree = null;
		Node newRoot = null;
		
		for (int test=0; test<8; test++) {
			if ((test & 1) == 0) {
				stdout.printf("\nTest %d: starting from tree with ternary root:\n",test);
				rerootTree = ternaryRootTree.getCopy();
			} else {
				stdout.printf("\nTest %d: starting from tree with binary root:\n",test);
				rerootTree = binaryRootTree.getCopy();
			}
			TreeUtils.printNH(rerootTree, stdout, true, false);
			if ((test & 2) == 0) {
				stdout.print("Rerooting to (A,B) internal node"); 
				newRoot = TreeUtils.getNodeByName(rerootTree.getRoot(),"A").getParent();
			} else {
				stdout.print("Rerooting to 'A' leaf node"); 
				newRoot = TreeUtils.getNodeByName(rerootTree.getRoot(),"A");
			}
			if ((test & 4) == 0) {
				stdout.println(" using basic reroot method yields tree");
				reroot(rerootTree,newRoot);
			} else {
				stdout.println(" using binary reroot method yields tree");
				rerootBinary(rerootTree,newRoot);
			}
			TreeUtils.printNH(rerootTree, stdout, true, false);
		}
	}
