...
// Comments prefixed 'MDW:' have been added by Michael Woodhams as I try to understand the code.
...
/*
 * MDW: Comments starting 'MDW' are by Michael Woodhams, added when trying to understand how this code works.
 * 
 * High level view of how it works:
 * The algorithm calculates partial likelihoods at each node, such that no partial likelihood ever needs 
 * recalculating and no unnecessary partial likelihood is ever calculated. It does this at a cost of being 
 * very memory intensive.  
 * 
 * The core data object is the NNode, being a node in the tree with additional partial likelihood calculations
 * attached.
 * 
 * Each NNode knows:
 * + Where it is in the tree (pointer to corresponding node in the tree passed to GeneralLikelihoodCalculator)
 * + The site patterns which are exhibited by the subtree of descendants of this node
 * + For each sequence site, which of those patterns it exhibits
 * + For each site pattern P, for each transition category, for each state S: the probability that pattern P
 * appears at the leaves AND that state S appears at the top of the branch above this node (i.e. one step
 * closer to the root.)
 * 
 * The calculations are all done recursively starting at the root. First the patterns are assembled, then
 * likelihoods can be calculated. The code is designed to allow many recalculations of the likelihood, each 
 * time only recalculating the minimum necessary. (If the model has not changed and the branch lengths in a
 * given subtree have not changed, then the partial likelihoods of that subtree are not recalculated.)
 * 
 * There are two fundamental types of NNodes: LeafNode and InternalNode. There are three additional NNode types:
 * FourStateInternalNode, BificatingInternalNode and FourStateBificatingInternalNode. These exist purely for
 * optimizing the common cases.
 * 
 * Some further optimizations which could be done:
 * + When assembling the list of unique patterns at each node, the pattern at each site is compared *by linear 
 * search* against a list of already observed patterns. This is inefficient when the number of patterns is large,
 * and could be done by hashing instead.
 * + The model can have multiple 'transition categories' (usually rate classes). If the rate matrix for any
 * one of these changes, all numbers are recalculated, even for transition categories which have not had rate 
 * matrix changes.
 * 
 * Note: I suspect that this code implicitly assumes a time reversible model, but the algorithm need not
 * be so constrained.
 */
...
public class GeneralLikelihoodCalculator implements PalObjectListener, LikelihoodCalculator {
	...
	int numberOfSites_;
	int numberOfStates_;
	// MDW: Number of rate matrices in the model: commonly only one, or implementing gamma distribution of rates.
	int numberOfTransitionCategories_;
	// MDW: this is just preallocated working memory
	...
	public final void setup(Tree t, SubstitutionModel model) {
		...
		// MDW: create an copy of the supplied tree (of Nodes) out of NNodes.
		if (root_ == null) {
			root_ = create(t.getRoot());
		}
		// could be more efficient
		// MDW: recursively generate the site pattern lists for each node
	...
	// MDW:
	// Given a node, create the corresponding NNode (of appropriate NNode sublcass) and then
	// recurse into subtree of node, creating NNode copies.
	 ...
	 */
	// MDW: at this point, all children of the root node have up-to-date partial likelihoods.
	// This does the final calculations to return overall likelihood and posterior TransitionCategory 
	// probabilites for each site.
	// Why doesn't this use NNode.calculateFinal? Much of the code is replicated there.
	private static final LikelihoodSummary calculateFinalSummaryImpl(
			DataType dt, 
			double[] equilibriumProbabilities, 
			int numberOfPatterns, 
			double[] categoryProbabilities, 
			int[] patternWeights, 
			double[][][][] childPatternProbs, 
			int[] patterns, 
	 ...
	 * MDW:
	 * Every NNode has:
	 * + An ordinary Node for which it is holding partial likelihood calculations (Node peer_)
	 * + the length of the branch above this node from last time this node was calculated (so we know whether
	 *   it has changed, to trigger recalculation)
	 * + partial likelihood results patternStateProbabilities_[transitionCategory][patternNumber][state]
	 *   For submodel transitionCategory applied to pattern patternNumber, what is the probability of observing
	 *   this pattern and also that the far end of the branch above this node will be in 'state'.
	 * + Markov matrices for the edge above this node: transitionProbs[transitionCategory][state][state]
	 *   For leaf nodes, it is actually the transpose of the Markov matrix (which allows patternStateProbabilities 
	 *   in leaf nodes to simply contain pointers into transitionProbs)
	 * + (implicitly) the number of unique site patterns in this subtree, patternWeights_.length
	 * + The number of times each site pattern was observed, patternWeights[patternNumber]
	 * + For each site, which pattern number it exhibits, sitePatternMatchup_[numberOfSites]
	 * 
	 * The various InternalNode subclasses also have:
	 * + A list of patterns (logically patterns_[numberOfPatterns][numberOfChildren], but implemented as a
	 *   one dimensional array patterns_[].)
	 *   Specifies the patterns in terms of the pattern numbers in each child subtree.
	 * + Pointers to the child NNodes
	 * + the children's patternStateProbability matrices (redundant, but cached for efficiency)
	 * + the number of unique site patterns at this node (redundant (= pattern_.length), cached for efficiency)
	 * + endStateProbs is just working space.
	 */
	...
	abstract class NNode {
		...
		// MDW: updates transitionProbs_ array, if necessary.
		// Returns 'true' if values have changed, 'false' if no recalculation was necessary.
		protected boolean updateTransitionProbabilities() {
			if(modelChanged_||isBranchLengthChanged()) {
				...
				model_.getTransitionProbabilities(distance,null,transitionProbs_);
				lastLength_ = distance;
				return true;
			}
			return false;
		}
		// MDW: As updateTransitionProbabilities, but returns transpose matrix (used for LeafNodes.)
		protected boolean updateTransitionProbabilitiesTranspose() {
			if(modelChanged_||isBranchLengthChanged()) {
				double distance = peer_.getBranchLength();
				model_.getTransitionProbabilitiesTranspose(distance,null,transitionProbs_);
				lastLength_ = distance;
				return true;
		...
		// MDW: update patternStateProbabilities, return 'true' if recalculation has occurred
		abstract public boolean calculatePatternProbabilities();
		// MDW: Recursively find the site patterns (only needs to be done once) and allocate array with length
		// dependent on number of patterns. patternWeightStore is just working memory.
...
//==============================
	class LeafNode extends NNode {
		...
		// MDW: see comments on abstract class declaration
		...
		// MDW: Set up patterns for leaf nodes: count how many times each state occurs, 
		// set patternStateProbabilities_ as pointers into transitionProbs_ 
		...
		/*
		 * MDW:
		 * Returns new array: copy of input except any unrecognized states are mapped to a single value
		 * E.g. for four state data, states 0-3 are copied unchanged, anything else is copied as 4.
		 */
	...
	} // End of Class LeafNode extends NNode
	...
	class BificatingInternalNode extends NNode{
		...
		public void setupSequences(int[] patternWeightStore, int[][] states, Alignment base) {
			...
			for(int site = 0 ; site < numberOfSites_; site++) {
				...
				// MDW: linear search: inefficient when the number of patterns is large. 
				for(int pattern = 0 ; pattern < numberOfPatterns ; pattern++) {
			...
			// MDW: Allocate memory now we know how many patterns we have
			this.patternStateProbabilities_ = new double[numberOfTransitionCategories_][numberOfPatterns][numberOfStates_];
		}

		// MDW: returns true if either child had changes to patternStateProbabilities_
		...
		/*
		 * MDW: This method overridden in FourStateBificatingInternalNode (for speed)
		 * This is the core number crunching routine.
		 */

		public boolean calculatePatternProbabilities() {
			// MDW: If nothing has changed (child probability patterns, transition matrix on this edge) don't recalculate
			if(!setupProbabilityCalculate()) { return false; }
			int patternReadPoint;
			// MDW: For each transition category...
			for(int transitionCategory = 0 ; transitionCategory < numberOfTransitionCategories_ ; transitionCategory++) {
				// MDW: cache pointers, for speed:
				...
				patternReadPoint = 0;
				// MDW: For each pattern...
				for(int pattern = 0; pattern < numberOfPatterns_; pattern++) {
					// MDW: calculate likelihood of each state at this node
					...
					// MDW: Matrix multiplication to evolve endState back along the edge terminating in this node
		...
		// MDW: Overridden in FourStateBificatingInternalNode
		// Evaluate this at the root to turn partial likelihoods into an overall likelihood.
		// See comments to calculateFinalSummaryImpl.
	...
	// MDW: In the common case of four states (DNA/RNA), we can save time by hard-coding operations on
	// length 4 vectors instead of using loops.
	 ...
	 */
	// MDW: See comments on corresponding methods in BificatingInternalNode. 
	// Instead of left_ and right_ for children, now we need an array, otherwise no change.
	 ...
	 */
	// MDW: Again: allows hard-coding of operations on length 4 vectors, saving looping costs.
