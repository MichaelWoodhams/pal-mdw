...
public class SimpleLeafCalculator implements LHCalculator.Leaf{
	...
	private final double[][][] transitionProbabilityStore_; // [rateCategory][toState][fromState]
	...
	private SimpleLeafCalculator(SimpleLeafCalculator toCopy) {
		...
		this.transitionProbabilityStore_ = pal.misc.Utils.getCopy(toCopy.transitionProbabilityStore_);
		...
		this.conditionalProbabilities_ = createExtended(transitionProbabilityStore_, patternStateMatchup_,numberOfPatterns_,numberOfCategories_,numberOfStates_,parent_);
	...
	public SimpleLeafCalculator( int[] patternStateMatchup, int numberOfPatterns, int numberOfStates, int numberOfCategories, LHCalculator.Generator parent ) {
		...
		this.conditionalProbabilities_ = createExtended(transitionProbabilityStore_,patternStateMatchup,numberOfPatterns,numberOfCategories,numberOfStates,parent);
...
// ======================================
	...
	public ConditionalProbabilityStore getExtendedConditionalProbabilities( double distance, ModelEdgeParameters edgeParams, SubstitutionModel model, boolean modelChanged) {
		if(distance!=lastDistance_||lastDistance_<0||modelChanged) {
			model.getTransitionProbabilitiesTranspose( distance,edgeParams,transitionProbabilityStore_ );
