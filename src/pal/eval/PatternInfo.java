...
public final class PatternInfo {
  private final int[] sitePatternMatchup_; // MDW: For each site, which pattern number is it?
  private final int[] patternWeights_;     // MDW: For each pattern, how many times does it occur?
  private final int[] patternLookup_;      
  private int numberOfPatterns_;
  // patternLookup_ is a bit complex. Each 'entry' in patternLookup is a pattern it found.
  // For DNA data for a leaf, that means it will be a 0, 1, 2, 3 (A, C, G, T) or 4 (gap).
  // For an internal node, it is a pair of numbers, each being a pattern number for one
  // of the children - i.e. there was a site where left child had pattern number 3, 
  // right child had pattern number 15. Each pattern in patternLookup_ is unique - if
  // we find another one, we just increase the corresponding patternWeights_ entry.
	/**
	 * Cloning constructor
  ...
  // 'left' and 'right' labels here are arbitrary: swapping these two args would make no substantive difference.
