...
// -=-=--==-=-=-=---=-==-=--==-=-=-=-
	...
	private static final class ExternalImpl implements External {
	  ...
	  public void calculateSingleExtendedConditionals(double topBaseHeight, double bottomBaseHeight,
			...
			base_.calculateSingleExtendedIndirect(model_.getAdjustedDistance(topBaseHeight), null, model_,numberOfPatterns,baseConditionalProbabilities,resultConditionalProbabilities);
		}
		...
		public void calculateSingleDescendentExtendedConditionals(
		  double topBaseHeight, double bottomBaseHeight,
			...
			model_.setup(bottomBaseHeight,heightConverter_,false);
			base_.calculateSingleExtendedDirect(model_.getAdjustedDistance(topBaseHeight), null, model_,centerPattern.getNumberOfPatterns(),descendentConditionalProbabilities);
		}
		/**
     */
...
//			System.out.println("**** ASCENDENT 3************");
			...
			base_.calculateSingleExtendedDirect(model_.getAdjustedDistance(topBaseHeight), null, model_,centerPattern.getNumberOfPatterns(),ascendentConditionalProbabilityProbabilties);
		}
		/**
     */
...
//			System.out.println("**** ASCENDENT 2************");
			...
			model_.setup(bottomBaseHeight,heightConverter_,true);
			base_.calculateSingleExtendedIndirect(model_.getAdjustedDistance(topBaseHeight), null, model_,centerPattern.getNumberOfPatterns(),baseAscendentConditionalProbabilityProbabilties,resultConditionalProbabilityProbabilties);
		}
		...
		public void calculateExtendedConditionals( double topBaseHeight, double bottomBaseHeight,
			...
			model_.setup(bottomBaseHeight,heightConverter_,false);
		  base_.calculateExtended(model_.getAdjustedDistance(topBaseHeight), false, null, model_,centerPattern,leftConditionalProbabilities,rightConditionalProbabilities,resultStore);
		}
		/**
    ...
    public double calculateLogLikelihoodNonRoot(
      ...
      ConditionalProbabilityStore descendentConditionalProbabilitiesStore
			...
			return base_.calculateLogLikelihood(model_,centerPattern,ascendentConditionalProbabilitiesStore,descendentConditionalProbabilitiesStore,null);
		}
    ...
    public double calculateLogLikelihood( double rootHeight, PatternInfo centerPattern,
			...
			return  base_.calculateLogLikelihood(model_,centerPattern,leftConditionalProbabilitiesStore,rightConditionalProbabilitiesStore,null);
		}
...
// -=-=--==-=-=-=---=-==-=--==-=-=-=-
	...
	private static final class LeafImpl implements Leaf {
		...
		public ConditionalProbabilityStore calculateExtendedConditionals(double topBaseHeight, double bottomBaseHeight) {
		  model_.setup(bottomBaseHeight,heightConverter_,false);
		    // MDW 'null' added to accomodate change in routine definition. 
			return base_.getExtendedConditionalProbabilities(model_.getAdjustedDistance(topBaseHeight),null,model_,true);
		}
    ...
    public ConditionalProbabilityStore calculatePostExtendedFlatConditionals( double topBaseHeight, double bottomBaseHeight, PatternInfo centerPattern,
      ...
      ConditionalProbabilityStore rightConditionalProbabilityProbabilties 	) {
		  model_.setup(bottomBaseHeight,heightConverter_,false);
		  return base_.calculatePostExtendedFlat(model_.getAdjustedDistance(topBaseHeight), null, model_,centerPattern,leftConditionalProbabilityProbabilties,rightConditionalProbabilityProbabilties,true);
        ...
        final ConditionalProbabilityStore rightConditionalProbabilityProbabilties
		  ...
		  model_.setup(bottomBaseHeight,heightConverter_,false);
		  return base_.calculateExtended(model_.getAdjustedDistance(topBaseHeight), false, null, model_,centerPattern,leftConditionalProbabilityProbabilties,rightConditionalProbabilityProbabilties,true);
		}
...
//		  System.out.println("**** ASCENDENT 1************");
		  model_.setup(bottomBaseHeight,heightConverter_,true);
		  return base_.calculateExtended(model_.getAdjustedDistance(topBaseHeight), false, null, model_,centerPattern,ascenedentConditionalProbabilityProbabilties,otherConditionalProbabilityProbabilties,true);
		}
...
// -=-=-=-=-=
	  	public void getTransitionProbabilities(double branchLength, ModelEdgeParameters edgeParams, double[][][] tableStore) {
		  checkRebuild();	getTransitionProbabilitiesImpl(branchLength,tableStore, false);
		}
		public void getTransitionProbabilities(double branchLength, ModelEdgeParameters edgeParams, int category, double[][] tableStore) {
	    checkRebuild();	getTransitionProbabilitiesImpl(branchLength,category,tableStore,false);
		}

		public void getTransitionProbabilitiesTranspose(double branchLength, ModelEdgeParameters edgeParams, double[][][] tableStore) {
		  checkRebuild();	getTransitionProbabilitiesImpl(branchLength,tableStore, true);
		}
		public void getTransitionProbabilitiesTranspose(double branchLength, ModelEdgeParameters edgeParams, int category, double[][] tableStore) {
