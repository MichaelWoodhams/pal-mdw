 ...
 * 
 * MDW: Note:
 * I've found this class unreliable - on rare occasions, it can fail with 'QR failed' error or
 * it generates NaNs which cause EigenSystem to fail. I recommend accessing it via 'try/catch'
 * and falling back on OrthogonalSearch in case of failure. (Or debugging the class to find the 
 * real problem, which I'm too lazy to do.)
 */
public class ConjugateDirectionSearch extends MultivariateMinimum
{
	...
	/*
	 * Setting the seed gives replicability for debugging.
	 */
	public ConjugateDirectionSearch(long seed)
	{
		// random number generator
	...
	private double flin(double l, int j, double f)
	{
		...
		numFun++;
		double x=fun.evaluate( tflin );
		if (Double.isInfinite(x)||Double.isNaN(x)) {
			StringBuffer errmsg = new StringBuffer("ConjugateDirectionSearch: function returns non-numeric value for arguments (");
			errmsg.append(tflin[0]);
			for (int i=1; i<dim; i++) errmsg.append(",").append(tflin[i]);
			errmsg.append(")");
