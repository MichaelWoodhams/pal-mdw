...
public class NumericalDerivative
{
	...
	public static double secondDerivative(UnivariateFunction f, double x)
	{
		...
		// Centered second derivative, unless bounds on argument interfere.
		if (x+h<=f.getUpperBound() && x-h>=f.getLowerBound()) {
			return (f.evaluate(x + h) - 2.0*f.evaluate(x) + f.evaluate(x - h))/(h*h);
		} else {
			double xPlusUpperH = Math.min(x+h, f.getUpperBound());
			double upperH = Math.min(h, f.getUpperBound()-x);
			double fPrimeU = (f.evaluate(xPlusUpperH)-f.evaluate(x))/upperH;
			double xMinusLowerH = Math.max(x-h, f.getLowerBound());
			double lowerH = Math.min(h,x-f.getLowerBound());
			double fPrimeL = (f.evaluate(x)-f.evaluate(xMinusLowerH))/lowerH;
			return (fPrimeU-fPrimeL)/((upperH+lowerH)*0.5);
		}
	}
