...
public class UnivariateMinimum
{
	 ...
	 * @return  position of minimum
	 */
	private static final double[] searchPoints = new double[]{0.5,0.25,0.75,0.375,0.625,0.125,0.875};
	public double optimize(UnivariateFunction f, double tol, double lowerBound, double upperBound)
	{
		numFun = 2;
		
		// MDW: Added lots of complexity to ensure we're not getting NaNs.
		double fmin = f.evaluate(lowerBound);
		double fmax = f.evaluate(upperBound);
		if (Double.isNaN(fmin) || Double.isNaN(fmax)) {
			// Try to find a good point within the range. This is pretty crude:
			double guess=0;
			boolean foundNotNaN = false;
			for (double location : searchPoints) {
				guess = lowerBound + location*(upperBound-lowerBound);
				numFun++;
				if (!Double.isNaN(f.evaluate(guess))) {
					foundNotNaN = true;
					break;
				}
			}
			if (!foundNotNaN) throw new RuntimeException("Could not find a value of the parameter to avoid function evaluating to NaN");
			if (Double.isNaN(fmin)) {
				lowerBound = binSearchNotNaN(lowerBound,guess,f);
				fmin = f.evaluate(lowerBound);
			}
			if (Double.isNaN(fmax)) {
				upperBound = binSearchNotNaN(upperBound,guess,f);
				fmax = f.evaluate(upperBound);
			}
		}
		// MDW: End of added complications
		
		return minin(lowerBound, upperBound, fmin, fmax, f, tol);
	}
	/**
	 * The actual optimization routine (Brent's golden section method)
	 *
	...
	/**
	 * 
	 * @param bad f(bad) is NaN
	 * @param good f(good) is not NaN
	 * @param f function
	 * @return x such that x lies between good and bad, f(x) is not NaN, but moving
	 * slightly further away from 'good' does give NaN. 
	 * 
	 * The assumption is that f has a connected range within which it gives good values,
	 * and outside that range it gives NaN. We also assume that near the boundary,
	 * we're not going to be returning optimal values, so we don't need to 
	 * find that boundary with great accuracy.
	 */
	static final int NUM_BISECTIONS = 7; 
	double binSearchNotNaN(double bad, double good, UnivariateFunction f) {
		for (int i=0; i<NUM_BISECTIONS; i++) {
			double guess = (bad+good)/2;
			numFun++;
			if (Double.isNaN(f.evaluate(guess))) {
				bad = guess;
			} else {
	...
	private double[] bracketize(double min, double a, double max, UnivariateFunction f)
	{
		...
		// MDW: add guards against NaN's at the edges of the range
		double fa = f.evaluate(a); numFun++;
		if (Double.isNaN(fa)) 
			throw new RuntimeException("Function is NaN at starting point");
		double fmin = f.evaluate(min); numFun++;
		if (Double.isNaN(fmin)) min = binSearchNotNaN(min,a,f);
		double fmax = f.evaluate(max); numFun++;
		if (Double.isNaN(fmax)) max = binSearchNotNaN(max,a,f);
