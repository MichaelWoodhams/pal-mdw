...
public class Utils {
	...
	/**
	 * 
	 * @param input - a string such as "1,3,5,9-12"
	 * @return a list of integers, such as {1,3,5,9,10,11,12}
	 * @throws ParseException
	 */
	public static int[] parseNumberList(String input) throws ParseException {
		// quick and dirty 'enumerator'
		final int STARTOFLINE = 0;
		final int NUMBER = 1;
		final int COMMA = 2;
		final int HYPHEN = 3;
		...
		// First and last chars are digits.
		// All non-digits chars are are '-' or ','
		// All non-digits appear alone (i.e. separated by digits from each other)
		// 'adjacent' (i.e. separated only by digits) non-digits cannot both be '-'.
		// When considered as numbers, the number before a '-' is smaller than the number after it.
		
		
		// NOTE: as implemented, it just IGNORES any chars other than [0-9,-] rather than objecting. 
		Pattern pattern = Pattern.compile("\\d+|[,-]");
		Matcher matcher = pattern.matcher(input);
		int lastToken = STARTOFLINE;
		int lastDelim = STARTOFLINE;
		int lastNumber = 0; // this value never used.
		while (matcher.find()) {
			String token = matcher.group();
			if (token.equals("-")) {
				if (lastToken != NUMBER) throw new ParseException("'-' must follow a number", matcher.start());
				if (lastDelim == HYPHEN) throw new ParseException("Two '-'s in a row", matcher.start());
				lastToken = HYPHEN;
				lastDelim = HYPHEN;
				...
				int thisNumber = Integer.parseInt(token); 
				if (lastToken == HYPHEN) {
					if (thisNumber < lastNumber) throw new ParseException("Number range must have lower number first",matcher.start());
					for (int i=lastNumber+1; i<=thisNumber; i++) list.add(i);
				} else {
					list.add(thisNumber);
				}
				lastToken = NUMBER;
		...
		if (lastToken != NUMBER) throw new ParseException("String must end with a number", input.length());
	 ...
	 * @return the sum of all the elements
	 */
	public static final int getSum(int[] array) {
		int total = 0;
		for(int i = 0 ; i < array.length ; i++) {
		  total+=array[i];
		}
