 ...
 * 
 * MDW: SEE ALSO NEOPARAMETERIZED, PARAMETERIZEDNEOWRAP which seem to be intended as replacements for this.
 * (Why they didn't just make NeoParameterized extend Parameterized is beyond me.) 
 * MDW: Suggested TODO: double[] getAllParameters(), void getAllParameters(double[]) and void setAllparameters(double[]) into Parameterized. 
 * 
 */
...
// ============================================================================
	...
	abstract public static class ParameterizedUser extends PalObjectListener.EventGenerator {
		...
		protected double getRandomParameterValue(int parameter) {
			...
			return MersenneTwisterFast.getNextDouble()*(max-min)+min;
		}
		...
		public final void setAllParameters(double[] parameters) {
			if (parameters.length != getNumParameters()) { throw new IllegalArgumentException("Parameter vector has wrong length"); }
			for(int i = 0 ; i < parameters.length ; i++) {
				base_.setParameter(parameters[i],i);
			}
		}
	}
